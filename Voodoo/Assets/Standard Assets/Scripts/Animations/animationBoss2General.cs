...
public class animationBoss2General : MonoBehaviour {
	int counter;
	float speed;
	int jumpTimer;
	int shootTimer;
	...
	void FixedUpdate () 
	{
		if (final) {
			...
			//camera = GameObject.FindWithTag ("MainCamera");//.GetComponent<Camera>()//orthographicSize -= .1f;
			//Camera.main.orthographicSize -= .1f;
				Vector3 pos = this.transform.position;
			if (deathCounter > 8)
				pos.Set (pos.x - .03f, pos.y + .02f, pos.z);
			else
				pos.Set (pos.x - .03f, pos.y - .02f, pos.z);
			...
			this.transform.Rotate (new Vector3 (0f, 0f, this.transform.rotation.z + 1f));
			deathCounter--;
			...
			if (counter >= 400 && counter <= 800) {
				...
				if (counter == 800) AudioSource.PlayClipAtPoint (grunt1, this.transform.position);
					...
					if (counter >= 10000 && counter < 10500) {
								speed = 0;
								swag.SetBool ("Selected", false);
				if (counter == 10020) AudioSource.PlayClipAtPoint (grunt1, this.transform.position);
				if (counter == 10100) AudioSource.PlayClipAtPoint (grunt2, this.transform.position);
				if (counter == 10300) AudioSource.PlayClipAtPoint (grunt3, this.transform.position);
				if (counter == 10400) AudioSource.PlayClipAtPoint (grunt4, this.transform.position);
	...
	void OnTriggerEnter2D (Collider2D other)
	{
		...
		if (other.gameObject.tag == "animationSpecial") {
			final = true;
				}
